...
# h = Attention()([x,x])
# h = Bidirectional(tf.keras.layers.LSTM(32, dropout=0.3))(h)
...
callback_list = [tf.keras.callbacks.ModelCheckpoint(filepath='/Users/wangdong/WorkSpace/MSA Datasets/SIMS/res_tmp/model.tf',
                                                    monitor='val_loss', save_best_only=False, save_freq='epoch', save_format='tf')]
...
unique_labels = np.unique(np.concatenate((nuneric_test_labels, predicted_test_labels)))
